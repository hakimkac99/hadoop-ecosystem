...
def scd1_merge(df_source, df_target, primary_keys):
    ...
    # All non-key columns
    other_columns = [c for c in df_target.columns if c not in primary_keys]
    ...
    # Conditions
    source_null_cond = all_null("source", other_columns)
    target_null_cond = all_null("target", other_columns)
    source_notnull_cond = any_not_null("source", other_columns)
    target_notnull_cond = any_not_null("target", other_columns)

    # New rows (in source only)
    new_rows = join_result.where(source_notnull_cond & target_null_cond).select(
        *primary_keys, *select_columns("source", other_columns)
    )
    ...
    rows_to_keep = join_result.where(source_null_cond & target_notnull_cond).select(
        *primary_keys, *select_columns("target", other_columns)
    )

    # Final result
    scd1_result = rows_to_keep.union(updated_rows).union(new_rows)
